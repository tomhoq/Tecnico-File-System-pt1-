#include "../fs/operations.h"
#include <assert.h>
#include <stdbool.h>
#include <stdint.h>
#include <stdio.h>
#include <string.h>
#include <pthread.h>
uint8_t const file_contents[] = "AAA!";
char const target_path1[] = "/f1";
char const link_path1[] = "/l1";
char const target_path2[] = "/f2";
char const link_path2[] = "/l2";

void assert_contents_ok(char const *path) {
    int f = tfs_open(path, 0);
    assert(f != -1);

    uint8_t buffer[sizeof(file_contents)];
    assert(tfs_read(f, buffer, sizeof(buffer)) == sizeof(buffer));
    assert(memcmp(buffer, file_contents, sizeof(buffer)) == 0);

    assert(tfs_close(f) != -1);
}

void assert_empty_file(char const *path) {
    int f = tfs_open(path, 0);
    assert(f != -1);

    uint8_t buffer[sizeof(file_contents)];
    assert(tfs_read(f, buffer, sizeof(buffer)) == 0);

    assert(tfs_close(f) != -1);
}

void write_contents(char const *path) {
    int f = tfs_open(path, 0);
    assert(f != -1);

    assert(tfs_write(f, file_contents, sizeof(file_contents)) ==
           sizeof(file_contents));

    assert(tfs_close(f) != -1);
}
void *f1(){
    {   
        char *str = "Ola O meu nome é Tomaz";
        int f = tfs_open(target_path1, TFS_O_CREAT);
        assert(f != -1);
        

        assert_empty_file(target_path1); // sanity check
        tfs_write(f, str, strlen(str));
        tfs_copy_from_external_fs(target_path1, target_path2);
        assert(tfs_close(f) != -1);
    }

    printf("Successful test1.\n");
    return NULL;
}
void *f2(){
    {
        int f = tfs_open(target_path1, TFS_O_CREAT);
        assert(f != -1);
        assert(tfs_close(f) != -1);

        assert_empty_file(target_path1); // sanity check
    }

    printf("Successful test2.\n");
    return NULL;
}
void *f3(){
    {
        char *str = "Ola";
        int f = tfs_open(target_path2, TFS_O_CREAT);
        assert(f != -1);
        tfs_write(f, str, strlen(str));
        assert(tfs_close(f) != -1);

        assert_empty_file(target_path1); // sanity check
    }

    printf("Successful test2.\n");
    return NULL;
}
int main() {
    pthread_t pid[3];
    
    assert(tfs_init(NULL) != -1);
    pthread_create(&pid[0],NULL,(void *)f1,NULL);
    pthread_create(&pid[1],NULL,(void *)f2,NULL);
    pthread_create(&pid[0],NULL,(void *)f3,NULL);
    pthread_join(pid[0],NULL);
    pthread_join(pid[1],NULL);
    pthread_join(pid[2],NULL);
    // Write to symlink and read original fil

    printf("FINISH.\n");
    
    return 0;
}
